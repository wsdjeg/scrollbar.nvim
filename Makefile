...
help:
	...
	@echo "  make test PATTERN=get_position           # Match test/**/*get_position*_spec.lua"
...
#   make test PATTERN=get_position  (shorthand for test/**/*get_position*_spec.lua)
test: install-deps
...
clean:
	...
	@rm -rf test/*.out
	@rm -rf *.swp
	@rm -rf /tmp/scrollbar_nvim_test_* 2>/dev/null || true
